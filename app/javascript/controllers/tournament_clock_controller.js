...
export default class extends Controller {
  static targets = [
    ...
    "smallBlind", "bigBlind", "ante", "nextLevelLabel", "nextLevelValues", "nextLevelDuration"
  ]
  ...
  connect() {
    ...
    if (!this.soundEnabled && this.soundDialogTarget.showModal) {
      this.soundDialogTarget.showModal()
    }
  }
  ...
  render() {
    ...
    this.timerTarget.classList.toggle(
      ...
      !this.overtimeValue && seconds > 0 && seconds <= 9
    )
  ...
  playEndingAlert(seconds) {
    if (this.overtimeValue || seconds <= 0 || seconds > 9) return
    if (this.alertedLevelId === this.currentLevelIdValue || !this.soundEnabled) return

    this.alertedLevelId = this.currentLevelIdValue
    this.audioTarget.currentTime = 0
    this.audioTarget.volume = 1
    this.audioTarget.play().then(() => {
      const playableDuration = this.audioTarget.duration - 3
      if (!Number.isFinite(playableDuration) || playableDuration <= 0) return

      this.audioStopTimer = window.setTimeout(() => {
        this.audioTarget.pause()
        this.audioTarget.currentTime = 0
